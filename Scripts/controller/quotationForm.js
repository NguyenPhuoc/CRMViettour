...
CKEDITOR.replace("insert-note-quotationform");
...
$("#btnEdit").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: '/QuotationFormManage/EditInfoQuotationForm',
        data: JSON.stringify(dataPost),
        contentType: "application/json; charset=utf-8",
        dataType: "html",
        success: function (data) {
            $("#data-quotationform").html(data);
            CKEDITOR.replace("edit-note-quotationform");
            $("#edit-company").select2();
            $("#edit-permission").select2();
            $("#modal-edit-quotationform").modal("show");
        }
    });
});
